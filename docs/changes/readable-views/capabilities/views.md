...
Every arrow shall carry a label. An arrow standing for one relation shall be
labelled with that relation's name; for several, with their names in id
order, the first three and a count of the rest. A relation without a name
shall be labelled with its interface's contract, or its id where it names no
interface; the model already warned about it when loaded.
...
externals marked apart, and, under the diagram, links to the page of every
shown element that has a view and to the page one level up. Every flowchart
shall be accepted by Mermaid's parser.
